"""
Fragen wurden dirket ins skript geschrieben und anmwerkungen für wietere to-dos
...
===========================================================
    ...
    Frage: Wie ist ganz "A" nun konstruiert? Auf welcher Grundlage wird die deskriptive analyse und Regression gemacht? 
    in A schaue ich mir sowohl Packages als auch die Projekte an? In den projekten werden die packages verwendet - aber bestimmt werden ncht alle packages ind en repos verwendet?
    ABER: wichtig: die packages sind gleichzeitig auch repos --> wie groß ist die überschneidung? Wie wird mit dem Überhang umgegangen?
    ...
    ----------------------------------------
      ...
      FALSCH --> KORREKT: depsProjects.repoData.topics --> qualitative und quantitative Analyse nötig um herauszufinden was der ANteil von "empty" ist und was beisiele für eingetragene Werte sind. 
      Tpoics sind vom Eigentümer frei wählbare Wörter: https://docs.github.com/de/repositories/managing-your-repositorys-settings-and-features/customizing-your-repository/classifying-your-repository-with-topics#about-topics
...
Frage: Was ist nun die nun die n-Grundlage für die deskriptive und die regressive Analyse? Werden nur die Packages verwendet die ein zugehöreiges repo haben?
wie werden die übrigen Projekte behandelt, die signal A2 erfüllen?
...
# =============================================================================
#Frage: Sind die unterscheidlichen Tiers nötig?
...
#   -> Nur Sensitivitaetscheck: Wie viele Pakete kommen zusätzlich dazu?
# Frage/Bitte: Erweiterung der Keywords für zur Maximierung
...
def main():
    ...
    pkg_deps  = db["depsPackagesDependencies"]
    proj_panel = db["depsProjectsPanel"]

    # =========================================================================
    # BLOCK 1 — GRUNDGROESSEN
    # =========================================================================
    # Was: Rohe Dokumentenzahlen pro Collection
    # Warum: Basisgroessen für alle Prozentangaben in der Thesis
    ...
    log.write("  Methode: case-insensitive Regex mit Separator-Varianten")
    log.write("  Beispiel: machine[\\s\\-_]?learning trifft alle drei:")
    log.write("    'machine learning' / 'machine-learning' / 'machine_learning'")

    n_has_desc = run_count(packages, {
        "_id.system": "PYPI",
        "packageInformation.description": {"$exists": True, "$nin": [None, ""]}
    }, "Pakete mit nicht-leerer description (Basis fuer Signal A)", log)
    ...
    if n_pypi_raw and n_has_desc:
        ...
        log.write(f"    Rest ({n_pypi_raw - n_has_desc:,} Pakete): kein Beschreibungstext vorhanden")

    log.write("\n  A2: High-Confidence-Keywords (Hauptmass Signal A)")
    log.write("  Treffer wenn mind. 1 der High-Conf-Patterns im Beschreibungstext steht.")
    ...
    log.write("  Zeigt: Wie viele ZUSAETZLICHE Treffer kommen durch weniger praezise Keywords?")
    log.write("  Fuer die Thesis: Hauptzahl = High-Conf. Medium zeigt Robustheit nach oben.")
    n_combined = run_count(packages, {
        "_id.system": "PYPI",
        "packageInformation.description": {"$regex": COMBINED_REGEX, "$options": "i"}
    }, "Pakete mit mind. 1 Keyword (High+Medium, Sensitivitaet)", log)
    results["signal_a_combined"] = n_combined
    if n_pypi_raw and n_combined and n_high:
        delta = n_combined - n_high
        log.write(f"    => {100*n_combined/n_pypi_raw:.2f}% aller Pakete")
        log.write(f"    => Delta zu High-Conf: +{delta:,} Pakete ({100*delta/n_high:.1f}% mehr)")

    log.write("\n  A4: Einzelne Keywords (fuer viz_03 und Thesen-Tabelle)")
    log.write("  Zeigt welche Begriffe Entwickler am haeufigsten nutzen.")
    kw_counts = []
    for name, pattern in HIGH_CONF_KEYWORDS.items():
        n = run_count(packages, {
            "_id.system": "PYPI",
            "packageInformation.description": {"$regex": pattern, "$options": "i"}
        }, f"    keyword='{name}'  (pattern: {pattern})", log)
        if n:
            kw_counts.append({"keyword": name, "count": n, "pattern": pattern})
    ...
    # Frage: Wie genau ist dieser Fall gelöst? PRIO --> Werden nur Projekte betrachtete, die auch ein Package sind?
    # =========================================================================
    ...
    n_gh_has_desc = run_count(projects, {
        "type": "GITHUB",
        "description": {"$exists": True, "$nin": [None, ""]}
    }, "GitHub-Projekte mit nicht-leerer description", log)
    results["n_github_with_description"] = n_gh_has_desc
    if n_gh_raw and n_gh_has_desc:
        log.write(f"    => {100*n_gh_has_desc/n_gh_raw:.1f}% aller GitHub-Projekte haben Beschreibung")

    log.write("\n  A2a: High-Conf-Keywords in GitHub-Description (Hauptmass)")
    n_a2_high = run_count(projects, {
        "type": "GITHUB",
        "description": {"$regex": HIGH_CONF_REGEX, "$options": "i"}
    }, "GitHub-Projekte mit mind. 1 High-Conf-Keyword in description", log)
    results["signal_a2_github_high_conf"] = n_a2_high
    if n_gh_raw and n_a2_high:
        log.write(f"    => {100*n_a2_high/n_gh_raw:.2f}% aller GitHub-Projekte")
    ...
    kw_counts_gh = []
    for name, pattern in HIGH_CONF_KEYWORDS.items():
        n = run_count(projects, {
            "type": "GITHUB",
            "description": {"$regex": pattern, "$options": "i"}
        }, f"    github keyword='{name}'", log)
        if n:
    ...
    log.write("  SIGNAL B: Strukturelles Signal")
    log.write("  Wo: depsPackagesDependencies.dependencies[].name")
    log.write("  Das ist: Aus requirements.txt/setup.py/pyproject.toml extrahierte Deps")
    log.write("  depth=1: Nur direkte Abhaengigkeiten (Entwickler hat Library bewusst gewaehlt)")
    log.write("  depth=2+: Transitive Deps — werden NICHT gewaehlt (zu indirekt)")
    log.write("  Dedup: $group auf _id.name -> unique Pakete (nicht Versionen)")

    n_b_any = run_agg_count(pkg_deps, [
        {"$match": {
            "dependencies": {
                "$elemMatch": {"name": {"$in": ALL_AI_LIBS}, "depth": 1}
            }
        }},
        {"$group": {"_id": "$_id.name"}},
    ], "Unique Pakete mit mind. 1 AI-Lib als depth=1-Dep (alle Tiers)", log)
    results["signal_b_any_tier"] = n_b_any

    if n_deps_unique and n_b_any:
        log.write(f"    => {100*n_b_any/n_deps_unique:.2f}% aller Pakete im Dep-Graph")

    log.write("\n  B2: Per Tier (zeigt welche Library-Generation dominiert)")
    for tier_name, tier_libs in [("tier1", TIER1_DILHARA),
                                  ("tier2", TIER2_FOUNDATION),
                                  ("tier3", TIER3_ECOSYSTEM)]:
        n_tier = run_agg_count(pkg_deps, [
            {"$match": {
                "dependencies": {
                    "$elemMatch": {"name": {"$in": tier_libs}, "depth": 1}
                }
            }},
            {"$group": {"_id": "$_id.name"}},
        ], f"Unique Pakete mit {tier_name}-Library als depth=1-Dep", log)
        results[f"signal_b_{tier_name}"] = n_tier

    log.write("\n  B3: Pakete mit Libraries aus Tier1 UND Tier2 (Multi-Tier = komplexere AI-Projekte)")
    n_t12 = run_agg_count(pkg_deps, [
        {"$match": {
            "dependencies": {"$elemMatch": {"name": {"$in": TIER1_DILHARA}, "depth": 1}}
        }},
        {"$match": {
            "dependencies": {"$elemMatch": {"name": {"$in": TIER2_FOUNDATION}, "depth": 1}}
        }},
        {"$group": {"_id": "$_id.name"}},
    ], "Unique Pakete mit Tier1 UND Tier2 als depth=1-Deps (Multi-Tier)", log)
    results["signal_b_multi_tier"] = n_t12

    log.write("\n  B4: Top-20 haeufigstes AI-Libraries (fuer viz_05)")
    rows_libs = run_agg(pkg_deps, [
        {"$match": {
            "dependencies": {"$elemMatch": {"name": {"$in": ALL_AI_LIBS}, "depth": 1}}
        }},
        {"$unwind": "$dependencies"},
        {"$match": {"dependencies.name": {"$in": ALL_AI_LIBS}, "dependencies.depth": 1}},
        {"$group": {"_id": {"pkg": "$_id.name", "lib": "$dependencies.name"}}},
        {"$group": {"_id": "$_id.lib", "count": {"$sum": 1}}},
        {"$sort": {"count": -1}},
        {"$limit": 20},
    ], "Top-20 AI-Libraries nach Anzahl unique Pakete die sie nutzen", log, top_n=20)
    results["signal_b_top_libs"] = [
        {"lib": r["_id"], "count": r["count"],
         "tier": TIER_MAP.get(r["_id"], "unknown")}
        for r in rows_libs
    ]
#Frage: warum depth=1 und nicht depth=0 + depth=1?
#werden die Ki Libraries an sich inkludiert?  Bspw. torch?
    ...
    #      Signal A1 NICHT haben
    #   5. Union = Nur-A + Schnittmenge + Nur-B
    # Rückfrage: was ist mit den Projekten die Signal A2 erfülllen --> hier nur Betrachtung der A1 (Packages) und nicht der Projekte oder?
    #
    ...
    t0 = time.time()
    cursor_b = pkg_deps.aggregate([
        {"$match": {
            "dependencies": {"$elemMatch": {"name": {"$in": ALL_AI_LIBS}, "depth": 1}}
        }},
        {"$group": {"_id": "$_id.name"}},
    ], allowDiskUse=True)
    names_b = [doc["_id"] for doc in cursor_b if doc["_id"]]
    ...
    #Frage: von was sind diese daten? Sind es 37.970 Packages oder Projects die nun als KI Treatment Gruppe verwendet werden?
    # Gleiche Frage für die 486.639 Non-AI
    ...
    n_ai_topics_raw = run_count(proj_panel, {
        "repoData.topics": {"$in": AI_TOPICS}
    }, "Panel-Snapshots mit AI-Topic (raw, inkl. Mehrfach-Snapshots)", log)

    n_ai_topics_unique = run_agg_count(proj_panel, [
        {"$match": {"repoData.topics": {"$in": AI_TOPICS}}},
        {"$group": {"_id": "$name"}},
    ], "Unique Projekte mit AI-Topic (dedup via name='owner/repo')", log)
    ...
    log.write("\n  Top-20 AI-Topics (Snapshot-Basis, also gewichtet nach Panel-Haeufigkeit):")
    rows_topics = run_agg(proj_panel, [
        {"$match": {"repoData.topics": {"$in": AI_TOPICS}}},
        {"$unwind": "$repoData.topics"},
        {"$match": {"repoData.topics": {"$in": AI_TOPICS}}},
        {"$group": {"_id": "$repoData.topics", "count": {"$sum": 1}}},
        {"$sort": {"count": -1}},
        {"$limit": 20},
    ], "Top-20 AI-Topics in depsProjectsPanel", log, top_n=20)
    results["top_ai_topics"] = [{"topic": r["_id"], "count": r["count"]} for r in rows_topics]

    # =========================================================================
    # BLOCK 7 — ZEITLICHE VERTEILUNG
    # =========================================================================
    # Was: Wann wurden Pakete gegründet? AI-Anteil ueber Zeit?
    ...
    # Frage: Output sagt im Jahr 2005 gab es 30 veröffentlichungen --> Github gibt es offiziell erst seit 2008? Begründung notwendig
    # Frage: Welche AI Pakete werden verwendet? welches n? --> 
    # =========================================================================
    ...
    log.write("  Zeigt: Ist der AI-Anteil nach Nov. 2022 gestiegen?")
